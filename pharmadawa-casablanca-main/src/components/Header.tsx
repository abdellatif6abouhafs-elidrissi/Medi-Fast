...
import { Menu, X, Pill, Sun, Moon, User, LogOut, Languages } from "lucide-react";
...
const Header = () => {
  ...
  const menuItems = [
    ...
    ...(!isAdmin ? [
      ...
      { href: "/faqs", label: "الأسئلة الشائعة", isActive: location.pathname === "/faqs" },
    ] : []),
    // Only show order link for authenticated non-admin users
    ...(isAuthenticated && !isAdmin ? [
      { href: "/order", label: "طلب دواء", isActive: location.pathname === "/order" }
    ] : []),
    { href: "/pharmacy-partners", label: "الصيدليات الشريكة", isActive: location.pathname === "/pharmacy-partners" },
    // Admin-only link
    ...(isAdmin ? [
      { href: "/admin/dashboard", label: "لوحة التحكم", isActive: location.pathname.startsWith("/admin") }
    ] : []),
    // User-only link
    ...(isAuthenticated && user?.role === 'user' ? [
      { href: "/dashboard", label: "حسابي", isActive: location.pathname === "/dashboard" }
    ] : []),
  ].filter(Boolean);
              ...
              >
                <Link
                  ...
                  className={`font-arabic transition-smooth hover:text-primary relative ${
                    item.isActive ? "text-primary font-semibold" : "text-foreground"
                  }`}
                >
                  {item.label}
                  {item.isActive && (
                  ...
                  >
                    <Link
                      ...
                      className={`block font-arabic p-3 rounded-lg transition-smooth hover:bg-accent ${
                        item.isActive ? "text-primary bg-accent font-semibold" : "text-foreground"
                      }`}
                    >
                      {item.label}
                    </Link>
